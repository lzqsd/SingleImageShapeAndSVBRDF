...
parser.add_argument('--dataRoot', default='../Data/test', help='path to real images')
parser.add_argument('--experiment', default=None, help='the path to store samples and models')
parser.add_argument('--modelRoot', default = None, help = 'the path to the trained models')
parser.add_argument('--epochId', default=5, type=int, help='the training epoch of the BRDF reconstruction model')
parser.add_argument('--modelRootGlob', default = 'check_globalillumination', help='the path to the trained model')
parser.add_argument('--epochIdGlob', default=17, type=int, help='the training epoch of the trained model')
...
opt.experiment = opt.modelRoot.replace('check', 'test')
...
for i, dataBatch in enumerate(brdfLoader):
    ...
    if j == 1 or j % 2000 == 0:
        ...
        vutils.save_image( ( (0.5*(imEBatch + 1) )**(1.0/2.2)*segBatch.expand_as(imBatch) ).data,
                '{0}/{1}_imE.png'.format(opt.experiment, j) )
        vutils.save_image( ( (0.5*(imP1Batch + 1) )**(1.0/2.2)*segBatch.expand_as(imBatch) ).data,
                '{0}/{1}_imP1.png'.format(opt.experiment, j) )
        vutils.save_image( ( (0.5*(imP2Batch + 1) )**(1.0/2.2)*segBatch.expand_as(imBatch) ).data,
                '{0}/{1}_imP2.png'.format(opt.experiment, j) )
        vutils.save_image( ( (0.5*(imP3Batch + 1) )**(1.0/2.2)*segBatch.expand_as(imBatch) ).data,
